...
const graphData = (stats) => {
    ...
    stats.forEach(el => {
        if(el.hasOwnProperty('isUp')) segregated_logs.push({ isUp : el.isUp, category : 'OK', logged_at : el.logged_at });
        if(el.hasOwnProperty('warn_isUp')) segregated_logs.push({ isUp : el.warn_isUp, category : 'Warning', logged_at : el.logged_at });
        if(el.hasOwnProperty('not_isUp')) segregated_logs.push({ isUp : el.not_isUp, category : 'Failure', logged_at : el.logged_at });
    });
    return segregated_logs;
}

const UrlMonitorAggregates = ({monitor, device_id, agent_id}) => {
    ...
    function fetchHourlyMonitors(device, agent_id, before, after){
        ...
        if(device && agent_id && monitor){
            ...
            secure_axios(
                ...
                {
                    ...{
                        api_path : '/api/url_monitor/fetch/trends/hourly',
                        api_method : 'post',
    ...
    function fetchDailyMonitors(device, agent_id, before, after){
        ...
        if(device && agent_id && monitor){
            ...
            secure_axios(
                ...
                {
                    ...{
                        api_path : '/api/url_monitor/fetch/trends/daily',
                        api_method : 'post',
                        monitor_ref : monitor.agent_id,
                        agent_id : agent_id,
                        ...(before) && {before : before},
...
export default UrlMonitorAggregates;