...
const graphData = (stats) => {
    ...
    stats.forEach(el => {
        if(el.hasOwnProperty('status')) segregated_logs.push({ status : el.status, category : 'OK', logged_at : el.logged_at });
        if(el.hasOwnProperty('warn_status')) segregated_logs.push({ status : el.warn_status, category : 'Warning', logged_at : el.logged_at });
        if(el.hasOwnProperty('not_status')) segregated_logs.push({ status : el.not_status, category : 'Failure', logged_at : el.logged_at });
    });
    return segregated_logs;
}

const UptimeMonitorAggregates = ({monitor, device_id, agent_id}) => {
    ...
    function fetchHourlyMonitors(device, agent_id, before, after){
        ...
        if(device && agent_id && monitor){
            ...
            secure_axios(
                ...
                {
                    ...{
                        api_path : '/api/uptime_monitor/fetch/trends/hourly',
                        api_method : 'post',
    ...
    function fetchDailyMonitors(device, agent_id, before, after){
        ...
        if(device && agent_id && monitor){
            ...
            secure_axios(
                ...
                {
                    ...{
                        api_path : '/api/uptime_monitor/fetch/trends/daily',
                        api_method : 'post',
                        monitor_ref : monitor.agent_id,
                        agent_id : agent_id,
                        ...(before) && {before : before},
...
export default UptimeMonitorAggregates;