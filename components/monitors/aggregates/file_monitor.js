...
import {PageHeader, Carousel, Checkbox, DatePicker, Select, Button, Switch, Divider} from "antd";
...
const slideHeader = {fontSize : '1.4rem', borderBottom : 'lightGray 1px solid'}
...
const FileMonitorAggregates = ({monitor, device_id, agent_id}) => {
    ...
    function fetchHourlyMonitors(device, agent_id, before, after){
        ...
        if(device && agent_id && monitor){
            ...
            secure_axios(
                ...
                {
                    ...{
                        api_path : '/api/file_monitor/fetch/trends/hourly',
                        api_method : 'post',
    ...
    function fetchDailyMonitors(device, agent_id, before, after){
        ...
        if(device && agent_id && monitor){
            ...
            secure_axios(
                ...
                {
                    ...{
                        api_path : '/api/file_monitor/fetch/trends/daily',
                        api_method : 'post',
                        monitor_ref : monitor.agent_id,
                        agent_id : agent_id,
                        ...(before) && {before : before},
    ...
    let chartConfig = {
        ...
        annotations : [
            ...
            },
            {
                type : "regionFilter",
                start : ['min', 0],
                end : ['min', monitor ? monitor.failure_cap : 'median'],
                color : "#cc3300",
            }
        ],
    ...
    const makeConfig = (data, yField, annotation_suffix) => {
        return {
            data : data,
            padding : 'auto',
            xField : 'logged_at',
            ...
            annotations : [
                {
                    type : "regionFilter",
                    start : ['min', monitor ? monitor[`warning_cap_${annotation_suffix}`] : 'median'],
                    end : ['max', monitor ? monitor[`failure_cap_${annotation_suffix}`] : 'median'],
                    color : "#ffcc00",
                },
                {
                    type: 'text',
                    position: ['min', `${monitor ? monitor[`warning_cap_${annotation_suffix}`] : 0}`],
                    content: 'Warning threshold',
                    offsetY: -4,
                    style: {
                      textBaseline: 'bottom',
                ...
                {
                    type: 'text',
                    position: ['min', `${monitor ? monitor[`failure_cap_${annotation_suffix}`] : 0}`],
                    content: 'Failure threshold',
                    offsetY: -4,
                    style: {
                      textBaseline: 'bottom',
                ...
                },
                {
                    type: 'line',
                    start : ['min', monitor ? monitor[`warning_cap_${annotation_suffix}`] : 'median'],
                    end : ['max', monitor ? monitor[`warning_cap_${annotation_suffix}`] : 'median'],
                    style: {
                      stroke: '#cca300',
                      lineDash: [5, 5],
                    },
                ...
                {
                    type: 'line',
                    start : ['min', monitor ? monitor[`failure_cap_${annotation_suffix}`] : 'median'],
                    end : ['max', monitor ? monitor[`failure_cap_${annotation_suffix}`] : 'median'],
                    style: {
                      stroke: '#cc3300',
                      lineDash: [5, 5],
    ...
    return (
            <div>
                ...
                <div style={{display : 'flex', justifyContent : 'space-between'}}>
                    <Button type="primary" onClick={() => slider.current.prev()} icon={<ArrowLeftOutlined />} >Previous slide</Button>
                    <Button type="primary" onClick={() => slider.current.next()} icon={<ArrowRightOutlined />} >Next slide</Button>
                </div>
                <Divider/>
                <Carousel ref={ref => slider.current = ref} autoplay={autoPlay} dots={true} arrows dotPosition="top" >
                    <div style={contentStyle}>
                    {/* <Button onClick={() => slider.current.goTo(3)}>Click me</Button> */}
                        <h2 style={slideHeader}>Hourly Aggregates</h2>
                        ...
                        {!hourlyLoading && 
                            ...
                            <h2>File count trends</h2>
                                <Line
                                {...(makeConfig(hourly, 'file_count_avg', 'file_count'))}
                                />
                            </div>
                            <div style={{marginBottom : '2em'}}>
                            <h2>Max file size trends</h2>
                            <Line
                            {...(makeConfig(hourly, 'max_size_avg', 'file_size'))}
                            />
                            </div>
                            </>
                        }
                    ...
                    <div style={contentStyle}>
                        <h2 style={slideHeader} >Daily Aggregates</h2>
                        <RightAlignedButtonWrapper style={{marginBottom : '20px'}}>
                        ...
                        {!dailyLoading && 
                            ...
                            <h2>File count trends</h2>
                                <Line
                                {...(makeConfig(daily, 'file_count_avg', 'file_count'))}
                                data={daily}
                                />
                            </div>
                            <div style={{marginBottom : '2em'}}>
                            <h2>Max file size trends</h2>
                            <Line
                            {...(makeConfig(daily, 'max_size_avg', 'file_size'))}
                            />
                            </div>
                            </>
                        }
                    ...
                    <div style={contentStyle}>
                        <h2 style={slideHeader} >Weekly Aggregates</h2>
                        <RightAlignedButtonWrapper style={{marginBottom : '20px'}}>
                        ...
                        {!weeklyLoading && 
                            ...
                            <h2>File count trends</h2>
                                <Line
                                {...(makeConfig(weekly, 'file_count_avg', 'file_count'))}
                                data={weekly}
                                />
                            </div>
                            <div style={{marginBottom : '2em'}}>
                            <h2>Max file size trends</h2>
                            <Line
                            {...(makeConfig(weekly, 'max_size_avg', 'file_size'))}
                            />
                            </div>
                            </>
                        }
                    ...
                    <div style={contentStyle}>
                        <h2 style={slideHeader} >Monthly Aggregates</h2>
                        <RightAlignedButtonWrapper style={{marginBottom : '20px'}}>
                        ...
                        {!monthlyLoading && 
                            ...
                            <h2>File count trends</h2>
                                <Line
                                {...(makeConfig(monthly, 'file_count_avg', 'file_count'))}
                                />
                            </div>
                            <div style={{marginBottom : '2em'}}>
                            <h2>Max file size trends</h2>
                            <Line
                            {...(makeConfig(monthly, 'max_size_avg', 'file_size'))}
                            data={monthly}
                            />
                            </div>
                            </>
...
export default FileMonitorAggregates;