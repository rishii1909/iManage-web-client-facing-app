...
const UrlMonitorSettings = ({ form }) => {
  ...
  return (
    <>
      ...
      <Form.Item name="timeout_failure" label="Timeout: Failure">
        <InputNumber name="timeout_failure" defaultValue={1000} />
      </Form.Item>

      <Form.Item name="timeout_warning" label="Timeout: Warning">
        <InputNumber name="timeout_warning" defaultValue={60} />
      </Form.Item>

      <Form.Item name="warning_content" label="Warning Content">
        <TextArea
          name="content_warning"
          onChange={(e) => setWarn_content(e.target.value)}
          placeholder="Warn if content not found"
        ></TextArea>
      </Form.Item>

      {warn_content && (
        <Form.Item name="warn_if_not_found" label="Warning Trigger condition">
          <Radio.Group defaultValue={true} buttonStyle="solid">
            <Radio.Button value={false}>Warn if found</Radio.Button>
            <Radio.Button value={true}>Warn if not found</Radio.Button>
          </Radio.Group>
        </Form.Item>
      )}

      <Form.Item name="failure_content" label="Failure Content">
        <TextArea
          name="content_failure"
          onChange={(e) => setFail_content(e.target.value)}
          placeholder="Fail if content not found"
        ></TextArea>
      </Form.Item>
      {fail_content && (
        <Form.Item name="fail_if_not_found" label="Failure Trigger condition">
          <Radio.Group defaultValue={true} buttonStyle="solid">
            <Radio.Button value={false}>Fail if found</Radio.Button>
            <Radio.Button value={true}>Fail if not found</Radio.Button>
          </Radio.Group>
        </Form.Item>
      )}
    </>
