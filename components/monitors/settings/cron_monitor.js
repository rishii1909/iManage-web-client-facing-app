    ...
    <>
      ...
      >
        <Select
          ...
          onChange={(val) => {
            setCronUser(val);
            fetchCronJobs(device, agent_id, val);
          }}
        >
          {cronUsers &&
            Object.keys(cronUsers).map((key) => {
              return (
                <Option value={key} key={key} user={cronUsers[key]}>
                  <div
      ...
      <Button
        onClick={() => fetchCronUsers(device, agent_id)}
        disabled={!(cronUsers == null)}
      >
        Retry
      </Button>
      <Form.Item
      ...
      >
        ...
        >
          {cronJobs &&
            Object.keys(cronJobs).map((key) => {
              return (
                <Option value={key} key={key} user={cronJobs[key]}>
                  <div>
                    {" "}
                    <Tag color="blue">{cronJobs[key].recurrence}</Tag>{" "}
                    {cronJobs[key].command}{" "}
                  </div>
                </Option>
              );
            })}
      ...
      <Button
        onClick={() => fetchCronJobs(device, agent_id, cronUser)}
        disabled={!(cronJobs == null)}
      >
        Retry
      </Button>
      <Form.Item
        ...
        rules={[
          {
            required: true,
            message: "Please enter a a location to access cron logs from.",
          },
        ]}
      >
        <Input />
