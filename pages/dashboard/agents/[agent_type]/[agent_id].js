...
import { formValToTypes } from "../../../../helpers/devices/dict";
...
const agent_view = () => {
  ...
  useEffect(async () => {
    if (agent_id) {
      ...
      await secure_axios(
        `/agents/enumerate/agent`,
        { agent_id: agent_id, show_creds: true },
        router,
        (response) => {
      ...
      (response) => {
        if (response.accomplished) {
          message
            ...
            .then(() =>
              router.push(`/dashboard/agents?agent_tab=${agent_type}`)
            );
        } else {
          message.error(
            response.response.message
  ...
  const on_finish = async (data) => {
    const req_data = { ...data, agent_id, type: formValToTypes[data.type] };
    const loading = message.loading("Updating agent...", 0);
    await secure_axios(
    ...
    <Agent_dashboard subdomain="Add Agent">
      ...
      >
        <Form.Item
          ...
          label="Type"
          rules={[
            { required: true, message: "Please select a remote agent type" },
          ]}
        >
          <Select placeholder="Select remote agent type">
        ...
        <Form.Item
          ...
          rules={[
            {
              required: true,
              message: "Please enter your agent's API endpoint.",
            },
          ]}
        >
      ...
      <Row align="middle" justify="space-between">
        ...
        <Col span={3} offset={2}>
          <Button type="danger" large block onClick={delete_agent}>
            Delete
          </Button>
        </Col>
      </Row>
